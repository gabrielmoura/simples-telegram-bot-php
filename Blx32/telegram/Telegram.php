...
/// Get the number of updates
    ...
    private function sendAPIRequest($url, array $content, $post = true)
    {
        ...
        if (class_exists('TelegramErrorLogger')) {
            TelegramErrorLogger::log(json_decode($result, true), $content, $this->folder);
        }
        return $result;
    }
    ...
    {
        if (in_array($this->UserID(), $this->adm)):
            return true;
        else:
            return false;
        endif;
    ...
    /**
     * @return mixed
     * Um novo membro foi adicionado ao grupo, informações sobre eles (este membro pode ser o próprio bot).
     * Retorna o User.
     */
    public function New_chat_member()
    {
    ...
    {
        return $this->data["message"]["new_chat_participant"];
    }

    /**
     * @return mixed
     * Se nçao funcionar tentar com o message;
     * Retorna o User
     */
    public function Left_chat_member()
    {
    ...
    {
        return $this->data["message"]["left_chat_participant"];
    }

    public function file($file_and_folder)
    {//Apresenta erro
       $x = new \CurlFile(realpath($file_and_folder));
        return $x;
    }

    public function file_write($file_and_folder, $content)
    {
        $fp = fopen($file_and_folder,"a+");
        $escreve = fwrite($fp, $content);
        fclose($fp);
        if ($escreve != null):return true;endif;
    }
